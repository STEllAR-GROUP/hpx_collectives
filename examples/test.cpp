...
#include "hpx_collectives/collectives.hpp"
...
int main()
{
    std::cout << "tree binomial broadcast" << std::endl;
    {
        ...
        hpx::utils::collectives::nonblocking_binomial_broadcast nbb{
            "nonblocking_binomial_broadcast"};
        nbb(N);

        std::cout << hpx::get_locality_id() << ' ' << N << std::endl;
    }
...
//             for (const auto& val : output)
//             {
//                 std::cout << upcxx::rank_me() << ' ' << val << std::endl;
//             }
//         }
//     }
//     upcxx::barrier();
